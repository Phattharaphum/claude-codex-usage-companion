...
支援的格式為 `txt`（每行一筆鍵值記錄）、`csv`（標頭後接資料列）與 `jsonl`（每行一個 JSON 物件）。變更格式時，設定的檔名副檔名會自動改為 `.txt`、`.csv` 或 `.jsonl`。記錄包含更新時間、供應商、成功或錯誤狀態、5 小時與每週剩餘百分比和重置時間、可用重置點數，以及任何重新整理錯誤。Antigravity 僅會寫入兩個權威共用配額歷程：`Antigravity-Gemini` 與 `Antigravity-ClaudeAndChatGPT`；不會寫入個別模型、帳號、方案或模型成員資料。舊版本建立的 CSV 會在下一次寫入時原地升級：舊資料列會完整保留，新欄位則留白。相對路徑會以應用程式的 XDG state 目錄為基準解析。
